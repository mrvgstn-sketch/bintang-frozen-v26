-- R13 Nota & Dana Titipan — cash reconciliation stale handling
...
begin
 ...
 set status='CORRECTION_REQUIRED',note=concat_ws(E'\n',nullif(note,''),'[AUTO STALE] '||coalesce(p_reason,'Pergerakan kas berubah setelah rekonsiliasi.'))
 where reconciliation_date=p_date and status='VERIFIED';
end $$;
...
begin
 ...
 select * into oldp from public.bf_entrusted_payouts where id=p_payout for update;
 if oldp.id is null or oldp.status='REVERSED' then raise exception 'NT_INVALID_PAYOUT_REVERSAL'; end if;
 update public.bf_entrusted_payouts set status='REVERSED',verification_note=concat_ws(E'\n',nullif(verification_note,''),'[REVERSAL] '||btrim(p_reason)),verified_by=auth.uid(),verified_at=now()
 where id=p_payout returning * into p;
 update public.bf_cash_movements set status='REVERSED'
 where source_ref_type='ENTRUSTED_PAYOUT' and source_ref_id=p_payout::text and status='POSTED';
 perform public.bf_nt_mark_cash_reconciliation_stale(oldp.payment_date,'Reversal pembayaran Dana Titipan '||oldp.payout_no||'.');
 perform public.bf_nt_event(p.note_id,'PAYOUT_REVERSED',p_reason,to_jsonb(oldp),to_jsonb(p),p.id::text);
 return p;
end $$;

revoke execute on function public.bf_nt_mark_cash_reconciliation_stale(date,text) from public,anon,authenticated;
revoke execute on function public.bf_nt_create_payout(uuid,numeric,date,text,text,text,jsonb,text,text) from public,anon;
revoke execute on function public.bf_nt_reverse_payout(uuid,text) from public,anon;
