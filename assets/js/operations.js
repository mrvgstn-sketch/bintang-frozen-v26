...
function transactionCenter(mode){
  ...
  const weightCell=(v="",i=0)=>`<div class="bf-weight-cell"><label>R${i+1}</label><div class="bf-weight-control"><input class="bf-weight-input" inputmode="decimal" value="${esc(v)}" placeholder="0"><button type="button" class="bf-weight-remove" aria-label="Hapus timbangan R${i+1}" title="Hapus timbangan">×</button></div></div>`;
  const weightInputs=(w=[],initial=false)=>{const a=Array.isArray(w)&&w.length?w:(initial?[""]:[]);return `<div class="bf-weight-grid">${a.map((v,i)=>weightCell(v,i)).join("")}</div><div class="bf-weight-empty" ${a.length?'hidden':''}>Belum ada catatan timbangan.</div><div class="bf-op-actions" style="margin-top:7px"><button type="button" class="bf-op-secondary bf-add-weight">＋ Kolom Timbang</button></div>`};
  function bindWeights(box,cb){
    const grid=box.querySelector('.bf-weight-grid'),empty=box.querySelector('.bf-weight-empty');
    const renumber=()=>{[...grid.children].forEach((cell,i)=>{const label=cell.querySelector('label'),btn=cell.querySelector('.bf-weight-remove');if(label)label.textContent=`R${i+1}`;if(btn){btn.setAttribute('aria-label',`Hapus timbangan R${i+1}`);btn.title=`Hapus timbangan R${i+1}`}});if(empty)empty.hidden=grid.children.length>0};
    const calc=()=>{renumber();const v=[...box.querySelectorAll('.bf-weight-input')].map(x=>num(x.value));cb?.(v,v.reduce((a,b)=>a+b,0))};
    const bindCell=cell=>{const input=cell.querySelector('.bf-weight-input'),remove=cell.querySelector('.bf-weight-remove');if(input)input.oninput=calc;if(remove)remove.onclick=()=>{const label=cell.querySelector('label')?.textContent||'Timbangan',raw=input?.value?.trim()||'';const msg=raw?`Hapus catatan timbangan ${label}?\n\nNilai: ${raw} kg\n\nTotal timbang akan dihitung ulang setelah catatan ini dihapus.`:`Hapus kolom timbangan ${label}?\n\nKolom ini belum mempunyai nilai.`;if(!confirm(msg))return;cell.remove();calc()}};
  ...
  function bindOutgoing(){
    ...
    root.querySelector('#bf-add-customer').onclick=()=>{groups.insertAdjacentHTML('beforeend',custGroup({group_id:uid(),pos_status:POS_STATUS.pending},groups.children.length));bindAll();renumberOutgoing();recalcOutgoing(groups)};bindAll()
  }
