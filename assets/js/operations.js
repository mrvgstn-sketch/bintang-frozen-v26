...
function page(title,sub){
  ...
  el.insertAdjacentHTML("beforeend",'<div class="bf-quick-scroll" aria-label="Gulir cepat"><button type="button" class="bf-op-secondary" data-scroll="top" aria-label="Gulir ke atas">↑</button><button type="button" class="bf-op-secondary" data-scroll="bottom" aria-label="Gulir ke bawah">↓</button></div>');
  el.querySelector('[data-scroll="top"]')?.addEventListener('click',()=>el.scrollTo({top:0,behavior:'smooth'}));
  el.querySelector('[data-scroll="bottom"]')?.addEventListener('click',()=>el.scrollTo({top:el.scrollHeight,behavior:'smooth'}));
  el.querySelector(".bf-op-back").onclick=()=>{window.BFRestoreInlineTally?.();el.remove()};return el;
}
...
function transactionCenter(mode){
  ...
  let outgoingDateMode="all",outgoingDateFrom="",outgoingDateTo="",outgoingCustomerQuery="",focusGroupId="",quickSaveGroupId="",pendingPosHandoffGroupId="",pendingPosWindow=null;
  let incomingDateMode="all",incomingDateFrom="",incomingDateTo="",incomingSupplierQuery="";
  const draftKey=`bf_recovery_draft_r13r_${incoming?'masuk':'keluar'}`;let draftTimer=null,recoveryPending=false;
  const txIdentity=r=>String(r?._bf_uid||r?.transaction_no||"");
  const txSnapshot=r=>({updated_at:String(r?.updated_at||""),version:Number(r?._bf_version||0),deleted_at:String(r?.deleted_at||"")});
  const sameSnapshot=(a,b)=>!!a&&!!b&&a.updated_at===b.updated_at&&a.version===b.version&&a.deleted_at===b.deleted_at;
  const readDraft=()=>storage.get(draftKey,null);const clearDraft=()=>storage.remove(draftKey);
  ...
  function inItem(it={},ii=0){const hasWeights=Array.isArray(it.timbangan)||Array.isArray(it.weights),w=Array.isArray(it.timbangan)?it.timbangan:(Array.isArray(it.weights)?it.weights:[]),invoice=it.invoice_weight_kg??it.invoice_weight??"";return `<div class="bf-in-item"><div class="bf-op-grid"><div class="bf-op-field"><label>Item ${ii+1}</label><input class="bf-in-item-name" list="bf-product-list" value="${esc(it.item||'')}"></div><div class="bf-op-field"><label>Satuan</label><input class="bf-in-item-unit" value="${esc(it.satuan||it.unit||'Kg')}"></div></div><div class="bf-invoice-compare"><div class="bf-op-field"><label>Berat Invoice Supplier (kg)</label><input class="bf-invoice-weight" inputmode="decimal" value="${esc(invoice)}" placeholder="Contoh 102,50"></div><div class="bf-invoice-summary"><span>Total Timbangan Real <b><span class="bf-in-item-total">0</span> kg</b></span><span>Selisih <b class="bf-invoice-diff">Belum dicatat</b></span><small class="bf-invoice-note">Isi berat invoice untuk membandingkan dengan timbangan real.</small></div></div><div class="bf-op-field"><label>Catatan Timbangan Real</label><div class="bf-in-item-weights">${weightInputs(w,!hasWeights)}</div></div><button type="button" class="bf-op-secondary bf-remove-in-item">Hapus Item</button></div>`}
  ...
  function bindIncoming(){const groups=root.querySelector('#bf-supplier-groups');function recalc(){let grand=0;groups.querySelectorAll('.bf-supplier-group').forEach(g=>{let st=0;g.querySelectorAll('.bf-in-item').forEach(it=>{const vals=[...it.querySelectorAll('.bf-weight-input')].map(x=>num(x.value)),t=vals.reduce((a,b)=>a+b,0),invoiceInput=it.querySelector('.bf-invoice-weight'),raw=invoiceInput?.value?.trim()||'',diffEl=it.querySelector('.bf-invoice-diff'),note=it.querySelector('.bf-invoice-note');it.querySelector('.bf-in-item-total').textContent=t.toLocaleString('id-ID',{maximumFractionDigits:3});if(raw){const diff=t-num(raw);if(diffEl){diffEl.textContent=`${diff>0?'+':''}${diff.toLocaleString('id-ID',{minimumFractionDigits:2,maximumFractionDigits:3})} kg`;diffEl.className=`bf-invoice-diff ${diff<0?'lower':diff>0?'higher':'equal'}`}if(note)note.textContent=diff<0?'Real lebih rendah dari invoice':diff>0?'Real lebih tinggi dari invoice':'Sesuai'}else{if(diffEl){diffEl.textContent='Belum dicatat';diffEl.className='bf-invoice-diff'}if(note)note.textContent='Isi berat invoice untuk membandingkan dengan timbangan real.'}st+=t});g.querySelector('.bf-supplier-total').textContent=st.toLocaleString('id-ID');grand+=st});root.querySelector('#bf-in-grand-total').textContent=grand.toLocaleString('id-ID')}
    ...
    function bindAll(){
      ...
      groups.querySelectorAll('.bf-invoice-weight').forEach(inp=>{if(inp.dataset.bound)return;inp.dataset.bound='1';inp.addEventListener('input',recalc)});
  ...
  const KASIR_PINTAR_URL="https://kasirpintar.co.id/";
  function posHandoffSummary(tx,g){
    const lines=[`BINTANG FROZEN — BARANG KELUAR`,`No: ${tx?.transaction_no||tx?._bf_uid||'-'}`,`Tanggal: ${businessDate(tx)||'-'}`,`Customer: ${g?.customer||'Customer tidak tercatat'}`,``];
    (Array.isArray(g?.items)?g.items:[]).forEach((it,i)=>{const total=(Array.isArray(it.timbangan)?it.timbangan:(Array.isArray(it.weights)?it.weights:[])).reduce((a,b)=>a+num(b),0);lines.push(`${i+1}. ${it.item||'Item tidak tercatat'}`,`   Qty timbang: ${total.toLocaleString('id-ID',{maximumFractionDigits:3})} kg`,`   Qty pesanan: ${String(it.qty??'-')} ${it.satuan||it.unit||''}`)});
    lines.push(``,`Status POS Bintang Frozen: ${posLabel(posStatus(g))}`,`Catatan ini harus diselesaikan manual di Kasir Pintar. Membuka aplikasi/website tidak berarti transaksi POS sudah tercipta.`);
    return lines.join('\n');
  }
  ...
  async function launchKasirPintar(tx,g,preopened=null){
    if(!tx||!g)return false;
    const copied=await copyTextSafe(posHandoffSummary(tx,g));
    const win=preopened&&!preopened.closed?preopened:null;
    let opened=!!win;if(win){try{win.opener=null;win.location.href=KASIR_PINTAR_URL}catch(_){opened=false}}else{try{opened=!!window.open(KASIR_PINTAR_URL,'_blank','noopener,noreferrer')}catch(_){opened=false}}
    alert(copied?(opened?'Data Customer sudah disalin. Kasir Pintar dibuka. Lanjutkan input transaksi secara manual; status POS belum diubah.':'Data Customer sudah disalin, tetapi tab Kasir Pintar diblokir browser. Status POS belum diubah.'):(opened?'Kasir Pintar dibuka, tetapi data tidak berhasil disalin. Status POS belum diubah.':'Data tidak berhasil disalin dan Kasir Pintar tidak dapat dibuka. Status POS belum diubah.'));
    if(!opened&&confirm('Buka situs resmi Kasir Pintar di tab ini?'))location.assign(KASIR_PINTAR_URL);
    return copied;
  }
  ...
  function custGroup(g={},gi=0){
    ...
    return `<section class="bf-customer-group" data-group-id="${esc(gid)}" data-pos-status="${esc(status)}" data-pos-entered-at="${esc(g.pos_entered_at||'')}" data-pos-entered-by-id="${esc(g.pos_entered_by_id||'')}" data-pos-entered-by-email="${esc(g.pos_entered_by_email||'')}" data-pos-entered-by-name="${esc(g.pos_entered_by_name||'')}">
      ...
      <div class="bf-customer-footer"><div><span>Total Customer</span><b><span class="bf-customer-total">${num(total).toLocaleString('id-ID')}</span> kg</b><small class="bf-pos-meta">${posMeta(g)}</small></div><div class="bf-op-actions"><button type="button" class="bf-op-primary bf-save-customer">💾 Simpan Catatan</button><button type="button" class="bf-op-secondary bf-pos-handoff-editor">🧾 Simpan & Buka Kasir Pintar</button><button type="button" class="bf-op-secondary bf-add-item">＋ Tambah Item</button>${editing?`<button type="button" class="bf-pos-action" data-pos-action="entered">${status===POS_STATUS.needs_update?'✓ Tandai Sudah Diperbarui di POS':'✓ Tandai Sudah Input POS'}</button>`:''}${status===POS_STATUS.entered&&editing?'<button type="button" class="bf-op-secondary bf-pos-reset">Tandai Perlu Input</button>':''}<button type="button" class="bf-op-secondary bf-remove-customer">Hapus Customer</button></div></div>
    </section>`;
  }
  ...
  function bindOutgoing(){
    ...
    function bindAll(){
      ...
      groups.querySelectorAll('.bf-save-customer').forEach(b=>b.onclick=()=>{const group=b.closest('.bf-customer-group');quickSaveGroupId=group?.dataset.groupId||'';focusGroupId=quickSaveGroupId;root.querySelector('#txsave')?.click()});
      groups.querySelectorAll('.bf-pos-handoff-editor').forEach(b=>b.onclick=()=>{const group=b.closest('.bf-customer-group'),gid=group?.dataset.groupId||'';if(!gid)return;pendingPosHandoffGroupId=gid;quickSaveGroupId=gid;focusGroupId=gid;try{pendingPosWindow=window.open('about:blank','_blank')}catch(_){pendingPosWindow=null}root.querySelector('#txsave')?.click()});
  ...
  function incomingDateMatches(r){const d=businessDate(r);if(incomingDateMode==="all")return true;if(incomingDateMode==="today")return d===today();if(incomingDateMode==="date")return !!incomingDateFrom&&d===incomingDateFrom;if(incomingDateMode==="range"){if(incomingDateFrom&&d<incomingDateFrom)return false;if(incomingDateTo&&d>incomingDateTo)return false;return !!(incomingDateFrom||incomingDateTo)}return true}
  function incomingSupplierMatches(r){const q=normSearch(incomingSupplierQuery);if(!q)return true;const names=Array.isArray(r?.suppliers)?r.suppliers.map(x=>x?.supplier):[r?.supplier];return names.some(x=>normSearch(x).includes(q))}
  function incomingHistoryControls(){return `<div class="bf-out-history-tools bf-in-history-tools"><div class="bf-out-filter-grid"><label>Filter Tanggal<select id="bf-in-date-mode"><option value="all" ${incomingDateMode==="all"?"selected":""}>Semua</option><option value="today" ${incomingDateMode==="today"?"selected":""}>Hari Ini</option><option value="date" ${incomingDateMode==="date"?"selected":""}>Pilih Tanggal</option><option value="range" ${incomingDateMode==="range"?"selected":""}>Rentang Tanggal</option></select></label><label class="bf-in-date-from">Dari<input id="bf-in-date-from" type="date" value="${esc(incomingDateFrom)}"></label><label class="bf-in-date-to">Sampai<input id="bf-in-date-to" type="date" value="${esc(incomingDateTo)}"></label><label>Cari Supplier<input id="bf-in-supplier-search" type="search" value="${esc(incomingSupplierQuery)}" placeholder="Nama Supplier..."></label></div><div class="bf-out-filter-actions"><button type="button" class="bf-op-secondary" id="bf-in-filter-reset">Reset Filter</button></div></div>`}
  ...
  function historyShell(){return incoming?`${incomingHistoryControls()}<div class="bf-op-table-wrap"><table class="bf-op-table"><thead><tr><th>No</th><th>Tanggal</th><th>Supplier / Supir</th><th>Item</th><th>Total Kg</th><th>Keterangan</th><th></th></tr></thead><tbody id="txbody"></tbody></table></div>`:`${outgoingHistoryControls()}<div id="txbody" class="bf-out-history"></div>`}
  function draftFormRow(){
    const txno=root.querySelector('#txno')?.value||'',date=root.querySelector('#txdate')?.value||today(),notes=root.querySelector('#txnotes')?.value||'';
    const row={transaction_no:txno,tanggal:date,keterangan:notes};
    if(incoming){
      row.supir=root.querySelector('#txdriver')?.value||'';row.ongkos_kirim=num(root.querySelector('#txshipping')?.value||0);
      row.suppliers=[...root.querySelectorAll('.bf-supplier-group')].map(g=>{let nota_fotos=[];try{nota_fotos=JSON.parse(g.querySelector('.bf-supplier-photo-preview')?.dataset.photos||'[]')}catch(_){nota_fotos=[]}return {supplier:g.querySelector('.bf-supplier-select')?.value||'',nota_fotos,nota_foto:nota_fotos[0]||'',items:[...g.querySelectorAll('.bf-in-item')].map(it=>{const raw=it.querySelector('.bf-invoice-weight')?.value?.trim()||'';return {item:it.querySelector('.bf-in-item-name')?.value||'',satuan:it.querySelector('.bf-in-item-unit')?.value||'Kg',invoice_weight_kg:raw===''?null:num(raw),timbangan:[...it.querySelectorAll('.bf-weight-input')].map(x=>num(x.value))}})}});
    }else{
      const baseById=new Map((editing?.customers||[]).map(g=>[customerGroupId(g),g]).filter(([id])=>id));
      row.customers=[...root.querySelectorAll('.bf-customer-group')].map(groupEl=>{const gid=groupEl.dataset.groupId||uid(),previous=baseById.get(gid)||{},driverSelect=groupEl.querySelector('.bf-driver-select'),paymentSelect=groupEl.querySelector('.bf-payment-method');return {...previous,group_id:gid,customer:groupEl.querySelector('.bf-customer-search')?.value.trim()||'',marketing:groupEl.querySelector('.bf-customer-marketing')?.value||'',driver_id:canManageDelivery()?(driverSelect?.value||''):(previous.driver_id||''),driver_name_snapshot:canManageDelivery()?(driverSelect?.selectedOptions?.[0]?.dataset?.driverName||''):(previous.driver_name_snapshot||''),payment_method:canManageDelivery()?(paymentSelect?.value||'unknown'):(previous.payment_method||'unknown'),items:[...groupEl.querySelectorAll('.bf-out-item')].map(it=>({item:it.querySelector('.bf-item-name')?.value||'',qty:it.querySelector('.bf-item-qty')?.value||'',satuan:it.querySelector('.bf-item-unit')?.value||'Kg',timbangan:[...it.querySelectorAll('.bf-weight-input')].map(x=>num(x.value))}))}});
    }
    return row
  }
  function captureDraftNow(){if(!root.querySelector('#txno'))return;const base=editing?txSnapshot(editing):null;storage.set(draftKey,{schema:'r13r-1',kind:editing?'edit':'new',transaction_id:editing?txIdentity(editing):'',base,updated_at:new Date().toISOString(),form:draftFormRow()})}
  function scheduleDraft(){if(recoveryPending)return;if(draftTimer){clearTimeout(draftTimer);draftTimer=null}try{captureDraftNow()}catch(err){console.warn('[R13R] draft capture failed',err)}}
  function showDraftRecovery(){const draft=readDraft();if(!draft?.form||draft.schema!=='r13r-1')return;recoveryPending=true;const panel=root.querySelector('#bf-transaction-panel');if(!panel||panel.querySelector('.bf-draft-recovery'))return;const banner=document.createElement('div');banner.className='bf-draft-recovery';const when=draft.updated_at?new Date(draft.updated_at).toLocaleString('id-ID'):'waktu tidak diketahui';banner.innerHTML=`<div><b>Catatan belum tersimpan ditemukan</b><span>${esc(draft.kind==='edit'?'Draft edit transaksi':'Draft transaksi baru')} • ${esc(when)}</span></div><div><button type="button" class="bf-op-primary" data-draft-restore>Lanjutkan Catatan</button><button type="button" class="bf-op-secondary" data-draft-discard>Buang Draft</button></div>`;panel.prepend(banner);
    banner.querySelector('[data-draft-discard]').onclick=()=>{if(!confirm('Buang catatan recovery yang belum tersimpan ini?'))return;clearDraft();recoveryPending=false;banner.remove()};
    banner.querySelector('[data-draft-restore]').onclick=()=>{if(draft.kind==='edit'){const latest=data.find(x=>txIdentity(x)===String(draft.transaction_id||''));if(!latest){alert('Transaksi asal draft tidak ditemukan. Draft tidak dipulihkan agar tidak membuat transaksi duplikat.');return}if(!sameSnapshot(draft.base,txSnapshot(latest))){if(confirm('Data transaksi sudah berubah sejak draft dibuat. Draft tidak dipulihkan agar perubahan server tidak tertimpa.\n\nBuang draft lama ini?')){clearDraft();recoveryPending=false;banner.remove()}return}editing=latest}else{editing=null;if(data.some(x=>String(x.transaction_no||'')===String(draft.form.transaction_no||'')))draft.form.transaction_no=nextTx(prefix,draft.form.tanggal||today(),data)}recoveryPending=false;render(draft.form)};
  }
  function render(r={}){
    const tx=r.transaction_no||nextTx(prefix,r.tanggal||today(),data);
    root.querySelector('#bf-op-content').innerHTML=`<div class="bf-work-tabs"><button class="bf-work-tab active" id="bf-tab-transaksi">📝 Catatan</button><button class="bf-work-tab" id="bf-tab-tally">⚖️ Tally Timbangan</button></div><div id="bf-transaction-panel"><div class="bf-op-kpis"><div><span>Total Catatan</span><b>${data.length}</b></div><div><span>Total Kg</span><b>${data.reduce((s,x)=>s+kg(x),0).toLocaleString('id-ID')} kg</b></div><div><span>Fungsi</span><b>Buku Catatan</b></div><div><span>Koneksi</span><b>${navigator.onLine?'Online':'Offline'}</b></div></div>${incoming?incomingForm(r,tx):outgoingForm(r,tx)}<div class="bf-op-actions"><button class="bf-op-primary" id="txsave">💾 Simpan Catatan</button><button class="bf-op-secondary" id="txtally">⚖️ Buka Tally</button>${editing?'<button class="bf-op-secondary" id="txcancel">Batal Edit</button>':''}</div><div class="bf-op-card">${historyShell()}</div></div><div id="bf-inline-tally" class="bf-inline-tally"></div>`;
    incoming?bindIncoming():bindOutgoing();
    root.querySelector('#txsave').onclick=async()=>{
      if(!can('edit_data')&&!can('add_row'))return deny('edit_data');if(recoveryPending)return alert('Pilih Lanjutkan Catatan atau Buang Draft terlebih dahulu agar catatan recovery tidak tertimpa.');const saveButton=root.querySelector('#txsave'),quickTarget=quickSaveGroupId;quickSaveGroupId='';captureDraftNow();saveButton.disabled=true;
      ...
      const prepared=window.BFPrepareTransactionSave?window.BFPrepareTransactionSave(old,row):row,idx=all.findIndex(x=>(x._bf_uid||x.transaction_no)===(prepared._bf_uid||prepared.transaction_no));idx>=0?all[idx]=prepared:all.push(prepared);save(key,all);audit(old?'edit':'add',incoming?'barang_masuk':'barang_keluar',prepared.transaction_no||prepared._bf_uid,old||null,prepared,{});window.dispatchEvent(new CustomEvent('bf:local-change',{detail:{key}}));window.BFCloud?.push?.();clearDraft();data=rows(key).filter(x=>!x.deleted_at);const handoffTarget=!incoming?pendingPosHandoffGroupId:'';pendingPosHandoffGroupId='';const handoffWindow=pendingPosWindow;pendingPosWindow=null;if(handoffTarget){const savedGroup=prepared.customers?.find(g=>customerGroupId(g)===handoffTarget);editing=prepared;focusGroupId=handoffTarget;render(editing);if(savedGroup)setTimeout(()=>launchKasirPintar(prepared,savedGroup,handoffWindow),0);else try{handoffWindow?.close?.()}catch(_){}}else if(quickTarget&&!incoming){editing=prepared;focusGroupId=quickTarget;render(editing);setTimeout(()=>alert(`Catatan ${prepared.customers?.find(g=>customerGroupId(g)===quickTarget)?.customer||'Customer'} tersimpan.`),0)}else{editing=null;render()}
    };
    const showTx=()=>{window.BFRestoreInlineTally?.();root.querySelector('#bf-transaction-panel').style.display='';root.querySelector('#bf-inline-tally').classList.remove('active');root.querySelector('#bf-tab-transaksi').classList.add('active');root.querySelector('#bf-tab-tally').classList.remove('active')},showTally=()=>{root.querySelector('#bf-transaction-panel').style.display='none';root.querySelector('#bf-tab-transaksi').classList.remove('active');root.querySelector('#bf-tab-tally').classList.add('active');window.BFMountInlineTally?.(incoming?'masuk':'keluar',root.querySelector('#bf-inline-tally'),showTx)};root.querySelector('#bf-tab-transaksi').onclick=showTx;root.querySelector('#bf-tab-tally').onclick=showTally;root.querySelector('#txtally').onclick=showTally;root.querySelector('#txcancel')?.addEventListener('click',()=>{clearDraft();editing=null;focusGroupId='';render()});
    if(incoming){
      const mode=root.querySelector('#bf-in-date-mode'),from=root.querySelector('#bf-in-date-from'),to=root.querySelector('#bf-in-date-to'),search=root.querySelector('#bf-in-supplier-search');
      const syncIncomingDate=()=>{const m=mode?.value||'all';root.querySelector('.bf-in-date-from')?.classList.toggle('bf-filter-hidden',!['date','range'].includes(m));root.querySelector('.bf-in-date-to')?.classList.toggle('bf-filter-hidden',m!=='range')};
      mode?.addEventListener('change',()=>{incomingDateMode=mode.value;if(incomingDateMode==='today'){incomingDateFrom='';incomingDateTo=''}else if(incomingDateMode==='date'&&!incomingDateFrom){incomingDateFrom=today();if(from)from.value=incomingDateFrom}syncIncomingDate();draw()});from?.addEventListener('change',()=>{incomingDateFrom=from.value;draw()});to?.addEventListener('change',()=>{incomingDateTo=to.value;draw()});search?.addEventListener('input',()=>{incomingSupplierQuery=search.value;draw()});root.querySelector('#bf-in-filter-reset')?.addEventListener('click',()=>{incomingDateMode='all';incomingDateFrom='';incomingDateTo='';incomingSupplierQuery='';render(editing||{})});syncIncomingDate();draw()
    }else{
  ...
  function draw(q){
    ...
    if(incoming){body.innerHTML=z.slice().reverse().map(r=>{let party=Array.isArray(r.suppliers)?r.suppliers.map(x=>x.supplier).filter(Boolean).join(', '):r.supplier;if(r.supir)party=[party,r.supir].filter(Boolean).join(' / ');const items=Array.isArray(r.suppliers)?r.suppliers.flatMap(x=>(x.items||[]).map(i=>i.item)).filter(Boolean).join(', '):r.item;return `<tr><td><b>${esc(r.transaction_no||'-')}</b></td><td>${esc(r.tanggal||'-')}</td><td>${esc(party||'-')}</td><td>${esc(items||'-')}</td><td>${kg(r).toLocaleString('id-ID')} kg</td><td>${esc(r.keterangan||r.catatan||'-')}</td><td><button class="bf-op-secondary txedit" data-id="${esc(r._bf_uid||r.transaction_no||'')}">Edit</button> <button class="bf-op-secondary txdel" data-id="${esc(r._bf_uid||r.transaction_no||'')}">Hapus</button></td></tr>`}).join('')||`<tr><td colspan="7">${incomingSupplierQuery?'Supplier tidak ditemukan.':incomingDateMode!=='all'?'Belum ada Barang Masuk pada tanggal/periode ini.':'Belum ada catatan.'}</td></tr>`
      ...
      body.querySelectorAll('.txposhandoff').forEach(b=>b.onclick=()=>{const tx=data.find(x=>(x._bf_uid||x.transaction_no)===b.dataset.tx),g=(DM.customerGroups(tx)||[]).find(x=>customerGroupId(x)===b.dataset.group);if(!tx||!g)return;launchKasirPintar(tx,g)});
  ...
  root.addEventListener('input',e=>{if(e.target.matches('#bf-in-date-mode,#bf-in-date-from,#bf-in-date-to,#bf-in-supplier-search,#bf-out-date-mode,#bf-out-date-from,#bf-out-date-to,#bf-out-customer-search,#bf-pos-filter'))return;scheduleDraft()});
  root.addEventListener('change',e=>{if(e.target.matches('#bf-in-date-mode,#bf-in-date-from,#bf-in-date-to,#bf-in-supplier-search,#bf-out-date-mode,#bf-out-date-from,#bf-out-date-to,#bf-out-customer-search,#bf-pos-filter'))return;scheduleDraft()});
  root.addEventListener('click',e=>{if(e.target.closest('.bf-add-weight,.bf-remove-last-weight,.bf-add-in-item,.bf-remove-in-item,#bf-add-supplier,.bf-remove-supplier,.bf-nota-remove,.bf-add-item,.bf-remove-item,#bf-add-customer,.bf-remove-customer'))setTimeout(scheduleDraft,0)});
  render();showDraftRecovery();
}
