...
function transactionCenter(mode){
  ...
  function normalizeCust(r){
    const g=DM.customerGroups(r).map(x=>({...x,group_id:customerGroupId(x)||uid(),pos_status:posStatus(x)}));
    return g.length?g:[{group_id:uid(),customer:"",marketing:"",pos_status:POS_STATUS.pending,items:[{item:"",qty:"",satuan:"Kg",timbangan:[]}]}]
  }
  function outItem(it={},ii=0){const w=Array.isArray(it.timbangan)?it.timbangan:(Array.isArray(it.weights)?it.weights:[]);return `<div class="bf-out-item"><div class="bf-item-heading">Item ${ii+1}</div><div class="bf-op-grid"><div class="bf-op-field"><label>Item</label><input class="bf-item-name" list="bf-product-list" value="${esc(it.item||'')}"></div><div class="bf-op-field"><label>Qty Pesanan</label><input class="bf-item-qty" inputmode="decimal" value="${esc(it.qty??'')}"></div><div class="bf-op-field"><label>Satuan</label><input class="bf-item-unit" value="${esc(it.satuan||it.unit||'Kg')}"></div></div><div class="bf-op-field"><label>Catatan Timbangan Real</label><div class="bf-item-weights">${weightInputs(w)}</div><div class="bf-line-total">Total Item: <span class="bf-item-total">0</span> kg</div></div>${ii?'<button type="button" class="bf-op-secondary bf-remove-item">Hapus Item</button>':''}</div>`}
  ...
  function bindOutgoing(){
    ...
    function bindAll(){
      ...
      groups.querySelectorAll('.bf-remove-item').forEach(b=>b.onclick=()=>{b.closest('.bf-out-item').remove();recalcOutgoing(groups)});
      groups.querySelectorAll('.bf-remove-customer').forEach(b=>b.onclick=()=>{b.closest('.bf-customer-group').remove();recalcOutgoing(groups)});
      ...
      recalcOutgoing(groups)
    }
    root.querySelector('#bf-add-customer').onclick=()=>{groups.insertAdjacentHTML('beforeend',custGroup({group_id:uid(),pos_status:POS_STATUS.pending},groups.children.length));bindAll();recalcOutgoing(groups)};bindAll()
  }
  ...
  function outgoingHistoryControls(){return `<div class="bf-pos-history-tools"><label>Status POS <select id="bf-pos-filter"><option value="all">Semua</option><option value="pending">Perlu Input POS</option><option value="entered">Sudah Input POS</option><option value="needs_update">Perlu Update POS</option></select></label><div id="bf-pos-summary" class="bf-pos-summary"></div></div>`}
  ...
  function render(r={}){
    ...
    root.querySelector('#txsave').onclick=async()=>{
      ...
      if(incoming){
        ...
        row.customers=[...root.querySelectorAll('.bf-customer-group')].map(groupEl=>{
          const gid=groupEl.dataset.groupId||uid(),previous=oldById.get(gid),g={group_id:gid,customer:groupEl.querySelector('.bf-customer-search')?.value.trim()||'',marketing:groupEl.querySelector('.bf-customer-marketing')?.value||'',items:[...groupEl.querySelectorAll('.bf-out-item')].map(it=>({item:it.querySelector('.bf-item-name').value,qty:it.querySelector('.bf-item-qty').value,satuan:it.querySelector('.bf-item-unit').value,timbangan:[...it.querySelectorAll('.bf-weight-input')].map(x=>num(x.value))}))};
    ...
    const showTx=()=>{window.BFRestoreInlineTally?.();root.querySelector('#bf-transaction-panel').style.display='';root.querySelector('#bf-inline-tally').classList.remove('active');root.querySelector('#bf-tab-transaksi').classList.add('active');root.querySelector('#bf-tab-tally').classList.remove('active')},showTally=()=>{root.querySelector('#bf-transaction-panel').style.display='none';root.querySelector('#bf-tab-transaksi').classList.remove('active');root.querySelector('#bf-tab-tally').classList.add('active');window.BFMountInlineTally?.(incoming?'masuk':'keluar',root.querySelector('#bf-inline-tally'),showTx)};root.querySelector('#bf-tab-transaksi').onclick=showTx;root.querySelector('#bf-tab-tally').onclick=showTally;root.querySelector('#txtally').onclick=showTally;root.querySelector('#txcancel')?.addEventListener('click',()=>{editing=null;render()});root.querySelector('#bf-pos-filter')?.addEventListener('change',()=>draw(root.querySelector('#txsearch').value));draw('');root.querySelector('#txsearch').oninput=e=>draw(e.target.value)
  }
  function historyCustomerBlock(tx,g){
    const status=posStatus(g),total=DM.groupTotal(g),gid=customerGroupId(g);return `<div class="bf-history-customer"><div class="bf-history-customer-main"><div><b>${esc(g.customer||'Customer tidak tercatat')}</b><small>${esc(g.marketing?`Marketing: ${g.marketing}`:'Tanpa Marketing')}</small></div><span class="bf-pos-badge ${posClass(status)}">${esc(posLabel(status))}</span></div><div class="bf-history-customer-meta"><span>${num(total).toLocaleString('id-ID')} kg</span><span>${esc(posMeta(g))}</span></div>${can('edit_data')||can('add_row')?`<div class="bf-history-pos-actions">${status!==POS_STATUS.entered?`<button class="bf-op-secondary txpos" data-tx="${esc(tx._bf_uid||tx.transaction_no||'')}" data-group="${esc(gid)}">✓ ${status===POS_STATUS.needs_update?'Sudah Diperbarui di POS':'Sudah Input POS'}</button>`:`<button class="bf-op-secondary txposreset" data-tx="${esc(tx._bf_uid||tx.transaction_no||'')}" data-group="${esc(gid)}">Tandai Perlu Input</button>`}</div>`:''}</div>`
  }
  function draw(q){
    const body=root.querySelector('#txbody');if(!body)return;const query=String(q||'').toLowerCase(),z=data.filter(r=>JSON.stringify(r).toLowerCase().includes(query));
    if(incoming){body.innerHTML=z.slice().reverse().map(r=>{let party=Array.isArray(r.suppliers)?r.suppliers.map(x=>x.supplier).filter(Boolean).join(', '):r.supplier;if(r.supir)party=[party,r.supir].filter(Boolean).join(' / ');const items=Array.isArray(r.suppliers)?r.suppliers.flatMap(x=>(x.items||[]).map(i=>i.item)).filter(Boolean).join(', '):r.item;return `<tr><td><b>${esc(r.transaction_no||'-')}</b></td><td>${esc(r.tanggal||'-')}</td><td>${esc(party||'-')}</td><td>${esc(items||'-')}</td><td>${kg(r).toLocaleString('id-ID')} kg</td><td>${esc(r.keterangan||r.catatan||'-')}</td><td><button class="bf-op-secondary txedit" data-id="${esc(r._bf_uid||r.transaction_no||'')}">Edit</button> <button class="bf-op-secondary txdel" data-id="${esc(r._bf_uid||r.transaction_no||'')}">Hapus</button></td></tr>`}).join('')||'<tr><td colspan="7">Belum ada catatan.</td></tr>'
      ...
      const filter=root.querySelector('#bf-pos-filter')?.value||'all';let pending=0,entered=0,needs=0;
      z.forEach(r=>(DM.customerGroups(r)||[]).forEach(g=>{const s=posStatus(g);if(s===POS_STATUS.entered)entered++;else if(s===POS_STATUS.needs_update)needs++;else pending++}));
      const summary=root.querySelector('#bf-pos-summary');if(summary)summary.innerHTML=`<span><b>${pending}</b> Perlu Input</span><span><b>${entered}</b> Sudah Input</span><span><b>${needs}</b> Perlu Update</span>`;
      const cards=z.slice().reverse().map(r=>{const groups=DM.customerGroups(r),visible=filter==='all'?groups:groups.filter(g=>posStatus(g)===filter);if(!visible.length)return '';const visibleKg=visible.reduce((sum,g)=>sum+DM.groupTotal(g),0);return `<article class="bf-out-history-card"><header><div><b>${esc(r.transaction_no||'-')}</b><span>${esc(r.tanggal||'-')}${filter!=='all'?` • ${visible.length}/${groups.length} Customer`:''}</span></div><strong>${visibleKg.toLocaleString('id-ID')} kg</strong></header><div class="bf-history-customers">${visible.map(g=>historyCustomerBlock(r,g)).join('')}</div><footer><span>${esc(r.keterangan||r.catatan||'')}</span><div><button class="bf-op-secondary txedit" data-id="${esc(r._bf_uid||r.transaction_no||'')}">Edit</button> <button class="bf-op-secondary txdel" data-id="${esc(r._bf_uid||r.transaction_no||'')}">Hapus</button></div></footer></article>`}).filter(Boolean);body.innerHTML=cards.join('')||'<div class="bf-op-empty">Belum ada Customer dengan status tersebut.</div>';
      body.querySelectorAll('.txpos').forEach(b=>b.onclick=async()=>{const tx=data.find(x=>(x._bf_uid||x.transaction_no)===b.dataset.tx),g=tx?.customers?.find(x=>customerGroupId(x)===b.dataset.group);if(!tx||!g)return;if(!confirm(`Tandai ${g.customer||'Customer'} sebagai sudah diinput/diperbarui di POS?\n\nTotal: ${DM.groupTotal(g).toLocaleString('id-ID')} kg`))return;await updatePosStatus(b.dataset.tx,b.dataset.group,POS_STATUS.entered);draw(root.querySelector('#txsearch').value)});
      body.querySelectorAll('.txposreset').forEach(b=>b.onclick=async()=>{const tx=data.find(x=>(x._bf_uid||x.transaction_no)===b.dataset.tx),g=tx?.customers?.find(x=>customerGroupId(x)===b.dataset.group);if(!confirm(`Tandai ${g?.customer||'Customer'} kembali sebagai Perlu Input POS?`))return;await updatePosStatus(b.dataset.tx,b.dataset.group,POS_STATUS.pending);draw(root.querySelector('#txsearch').value)})
    }
