...
function deleteSensitiveReasons(g={}){const out=[];if(Array.isArray(g.delivery_proofs)&&g.delivery_proofs.length)out.push('Delivery Proof');if(g.delivery_status==='delivered')out.push('Delivered');if(g.payment_method==='cash'&&['carried','handed_over'].includes(String(g.cash_status||'')))out.push('Cash '+g.cash_status);if(g.pos_status==='entered')out.push('Sudah Input POS');if(g.pos_status==='needs_update')out.push('Perlu Update POS');return out}
window.BFSoftDeleteTransaction=function(key,id){
  ...
  if(key==="bf_keluar_v26"){const sensitive=(Array.isArray(data[i].customers)?data[i].customers:[]).flatMap(g=>deleteSensitiveReasons(g).map(r=>(g.customer||'Customer')+': '+r));if(sensitive.length){alert('Safe Delete menolak transaksi ini karena state sensitif:\n\n• '+sensitive.join('\n• '));return false}}
