...
function deleteSensitiveReasons(g={}){const out=[];if(Array.isArray(g.delivery_proofs)&&g.delivery_proofs.length)out.push('Bukti Pengantaran');if(g.delivery_status==='delivered')out.push('Pengantaran Sudah Selesai');if(g.payment_method==='cash'&&String(g.cash_status||'')==='carried')out.push('Uang Tunai Sedang Dibawa Supir');if(g.payment_method==='cash'&&String(g.cash_status||'')==='handed_over')out.push('Uang Tunai Sudah Diserahkan');if(g.pos_status==='entered')out.push('Sudah Dicatat di POS');if(g.pos_status==='needs_update')out.push('Perlu Diperbarui di POS');return out}
window.BFSoftDeleteTransaction=function(key,id){
  ...
  if(key==="bf_keluar_v26"){const sensitive=(Array.isArray(data[i].customers)?data[i].customers:[]).flatMap(g=>deleteSensitiveReasons(g).map(r=>(g.customer||'Customer')+': '+r));if(sensitive.length){alert('Transaksi tidak dapat dihapus karena sudah memiliki proses penting yang tercatat:\n\n• '+sensitive.join('\n• '));return false}}
...
function boot(){
  ...
  window.BFRenderDashboardEnhancements?.();
}
