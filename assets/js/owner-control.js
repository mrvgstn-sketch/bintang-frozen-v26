...
function annotateTransactions(){
  ...
  ["bf_masuk_v23_manual","bf_keluar_v23_manual"].forEach(key=>{
...
window.BFSoftDeleteTransaction=function(key,id){
  ...
  window.BFLogActivity?.("delete",key==="bf_masuk_v23_manual"?"barang_masuk":"barang_keluar",data[i].transaction_no||id,null,data[i],{soft_delete:true});
  window.BFCloud?.push?.();
  return true;
};
...
window.BFRestoreTransaction=function(key,id){
  ...
  localStorage.setItem(key,JSON.stringify(data));
  window.BFLogActivity?.("restore",key==="bf_masuk_v23_manual"?"barang_masuk":"barang_keluar",data[i].transaction_no||id,null,data[i],{});
  window.BFCloud?.push?.();
  return true;
};
...
window.BFPermanentDeleteTransaction=function(key,id){
  ...
  window.BFLogActivity?.("permanent_delete",key==="bf_masuk_v23_manual"?"barang_masuk":"barang_keluar",old.transaction_no||id,old,null,{});
  window.BFCloud?.push?.();
  return true;
};
...
function addDeletedMenu(){
  const menus=document.querySelectorAll(".bf-final-menu-grid");
  menus.forEach(g=>{
    if(g.innerHTML.includes("Pengguna")&&!g.querySelector('[data-owner-action="deleted"]')){
      const b=document.createElement("button");b.className="bf-final-menu-item";b.dataset.ownerAction="deleted";b.innerHTML='♻️ Data Terhapus<small>Pulihkan / hapus permanen</small>';g.appendChild(b);
      b.onclick=()=>window.BFOpenDeletedData();
    }
  });
}
function boot(){
