...
function annotateTransactions(){window.BFOperationsRepository?.ensureMetadata?.()}
...
window.BFSoftDeleteTransaction=(key,id)=>window.BFOperationsRepository?.softDelete?.(key,id)===true;

window.BFRestoreTransaction=(key,id)=>window.BFOperationsRepository?.restore?.(key,id)===true;

window.BFPermanentDeleteTransaction=function(key,id){const me=currentProfile();if(me.role!=="owner"){alert("Hanya Owner yang dapat menghapus permanen.");return false}return window.BFOperationsRepository?.permanentDelete?.(key,id)===true};
