...
function annotateTransactions(){
  ...
  ["bf_masuk_v26","bf_keluar_v26"].forEach(key=>{
    ...
    if(changed)window.BFOperations?.commit?.(key,data,{source:"owner-annotation"});
  });
}
...
window.BFSoftDeleteTransaction=function(key,id){
  ...
  window.BFOperations.commit(key,data,{source:"soft-delete"});
...
window.BFRestoreTransaction=function(key,id){
  ...
  window.BFOperations.commit(key,data,{source:"restore"});
...
window.BFPermanentDeleteTransaction=function(key,id){
  ...
  if(i<0)return false;
  const old=data[i]; data.splice(i,1); window.BFOperations.commit(key,data,{source:"permanent-delete"});
