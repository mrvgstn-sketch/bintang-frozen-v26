...
async function writeKey(key,user){
  ...
  if(!b){
    ...
    if(r.error){if(r.error.code==="23505")return false;throw r.error}base.set(key,r.data);dirty.delete(key);return true;
  }
  ...
  if(r.error)throw r.error;if(!r.data)return false;base.set(key,r.data);dirty.delete(key);return true;
}
...
async function start(){
  ...
  finally{booting=false;if(authenticated){event("bf:cloud-ready",{userId:activeUserId});document.getElementById("bf-startup-screen")?.remove();try{window.BFMountMainApp?.()}catch(e){console.error("[Bintang Frozen] Aplikasi gagal dimuat",e)}}}
}
