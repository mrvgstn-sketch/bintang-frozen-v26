...
const KEYS=["bf_customers","bf_suppliers","bf_products","bf_employees","bf_debts","bf_expenses","bf_history","bf_keluar_v26","bf_masuk_fotos","bf_masuk_header","bf_masuk_v26","bf_note_pengeluaran_v26","bf_note_sembako_v26","bf_note_setoran_v26","bf_tally_ids","bf_tally_cols_v26","bf_deleted_transactions","bf_report_period"];
let cloud=null,channel=null,pollTimer=null,pushTimer=null,busy=false,booting=false,applyingRemote=false,activeUserId=null;
const dirty=new Set(),base=new Map();window.__bfSyncDirtyCount=0;
window.BF_STORE_CODE=STORE_CODE;window.BFCloudKeys=()=>KEYS.slice();
...
function applyRows(rows,{initial=false,realtime=false}={}){
  ...
  try{
    window.BFCore.storage.transaction(()=>{
      rows.forEach(r=>{if(!KEYS.includes(r.state_key))return;base.set(r.state_key,{value:r.value,revision:Number(r.revision||0),updated_at:r.updated_at||""});if(localValue(r.state_key)!==r.value){window.BFCore.storage.setRaw(r.state_key,r.value);changed.push(r.state_key)}dirty.delete(r.state_key);window.__bfSyncDirtyCount=dirty.size});
    });
...
function applyDelete(key){if(!KEYS.includes(key))return;applyingRemote=true;try{window.BFCore.storage.removeRaw(key);base.delete(key);dirty.delete(key);window.__bfSyncDirtyCount=dirty.size}finally{applyingRemote=false};event("bf:data-changed",{keys:[key],source:"cloud",deleted:true})}
...
async function writeKey(key,user){
  ...
  if(value===null){
    if(!b){dirty.delete(key);window.__bfSyncDirtyCount=dirty.size;return true}
    const r=await cloud.from(TABLE).delete().eq("store_code",STORE_CODE).eq("state_key",key).eq("revision",b.revision).select("state_key");
    if(r.error)throw r.error;if(!r.data?.length)return false;base.delete(key);dirty.delete(key);window.__bfSyncDirtyCount=dirty.size;return true;
  }
  if(!b){
    const r=await cloud.from(TABLE).insert({store_code:STORE_CODE,state_key:key,value,revision:1,updated_by:user.id}).select("state_key,value,revision,updated_at").maybeSingle();
    if(r.error){if(r.error.code==="23505")return false;throw r.error}base.set(key,r.data);dirty.delete(key);window.__bfSyncDirtyCount=dirty.size;return true;
  }
  const nextRevision=Number(b.revision||0)+1;
  const r=await cloud.from(TABLE).update({value,revision:nextRevision,updated_at:new Date().toISOString(),updated_by:user.id}).eq("store_code",STORE_CODE).eq("state_key",key).eq("revision",b.revision).select("state_key,value,revision,updated_at").maybeSingle();
  if(r.error)throw r.error;if(!r.data)return false;base.set(key,r.data);dirty.delete(key);window.__bfSyncDirtyCount=dirty.size;return true;
}
...
function watch(){if(window.__bfCloudStorageWatch)return;window.__bfCloudStorageWatch=true;window.BFCore.storage.onChange(({key})=>{if(KEYS.includes(key)&&!applyingRemote){dirty.add(key);window.__bfSyncDirtyCount=dirty.size;event("bf:local-change",{key});schedule()}})}
function stopPolling(){if(pollTimer){clearInterval(pollTimer);pollTimer=null}}
function signature(rows){return (rows||[]).map(r=>r.state_key+":"+r.revision).sort().join("|")}
function baseSignature(){return [...base.entries()].map(([k,v])=>k+":"+v.revision).sort().join("|")}
function polling(){if(pollTimer)return;pollTimer=setInterval(async()=>{try{if(!(await currentUser()))return;const rows=await readRemote();if(signature(rows)!==baseSignature())await pull(false)}catch(_){}},30000)}
function realtime(){
  if(channel)return;
...
window.BFCloud={keys:()=>KEYS.slice(),stateKeys:[...KEYS],storeCode:STORE_CODE,table:TABLE,pull:()=>pull(false),push,dirtyKeys:()=>[...dirty]};
window.addEventListener("bf:auth-ready",start);
window.addEventListener("bf:auth-signed-out",()=>{activeUserId=null;base.clear();dirty.clear();window.__bfSyncDirtyCount=0;clearTimeout(pushTimer);pushTimer=null;stopPolling();if(channel){try{cloud?.removeChannel(channel)}catch(_){}channel=null}event("bf:sync-status",{status:"signed-out"})});
})();
