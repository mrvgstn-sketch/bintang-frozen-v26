/*
  ...
  (async function(){
    ...
    const DEFAULTS = {
      ...
      driver: window.BFPermissions.defaults("driver")
    };
    ...
    async function loadPermissions(role){
      ...
      // R13N: hak Driver bersifat fixed/default-deny. Jangan izinkan tabel role_permissions
      // membuka permission lain secara tidak sengaja untuk akun Supir.
      if(role==="driver") return {...DEFAULTS.driver};
    ...
    function bridgeLegacySession(){
      ...
      const legacy={
        ...
        role:owner?"Owner":(profile.role==="admin"?"Admin":(profile.role==="driver"?"Supir":"Operator")),
    ...
    const AUDIT_ACTION_LABELS={
      ...
      permission_denied:"Akses Ditolak",update_permissions:"Mengubah Hak Akses",update_user_access:"Mengubah Akses Pengguna",update_display_name:"Mengubah Nama Karyawan",
      ...
      edit_supplier:"Mengubah Data Supplier",edit_customer:"Mengubah Data Customer",update_pos_status:"Mengubah Status Input POS",delivery_delivered:"Menandai Pengantaran Selesai",upload_delivery_proof:"Mengunggah Bukti Pengantaran",cash_carried:"Menandai Uang Dibawa Supir",cash_handed_over:"Mengonfirmasi Uang Diserahkan"
    };
    ...
    const AUDIT_FIELD_LABELS={
      ...
      commission_per_kg:"Komisi/Kg",permission:"Hak Akses",role:"Role",display_name:"Nama Karyawan",driver_id:"Supir",driver_name_snapshot:"Nama Supir",payment_method:"Pembayaran",delivery_status:"Status Pengantaran",cash_status:"Status Uang",delivery_proofs:"Bukti Pengantaran"
    };
    ...
    const auditRoleLabel=r=>({owner:"Owner",admin:"Admin",operator:"Operator",driver:"Supir"}[String(r||"").toLowerCase()]||String(r||"Pengguna"));
    ...
    async function openUsers(){
      ...
      const rows=(data||[]).map(x=>`<tr>
        <td>${esc(x.email)}</td><td><input data-name="${x.id}" value="${esc(x.display_name||"")}" maxlength="80" placeholder="Nama karyawan" style="min-width:170px;width:100%;padding:7px 9px;border:1px solid #dbe4ef;border-radius:9px"></td>
        <td><select data-role="${x.id}" ${x.role==="owner"?"disabled":""}>
          ${x.role==="owner"?'<option value="owner" selected>Owner</option>':''}
          <option value="admin" ${x.role==="admin"?"selected":""}>Admin</option>
          <option value="operator" ${x.role==="operator"?"selected":""}>Operator</option>
          <option value="driver" ${x.role==="driver"?"selected":""}>Supir / Driver</option>
        </select></td>
        <td><input type="checkbox" data-active="${x.id}" ${x.active?"checked":""} ${x.role==="owner"?"disabled":""}></td>
        <td><button data-save="${x.id}" class="bf-auth-btn" style="background:#0d1b3e;color:#fff">Simpan</button></td>
      </tr>`).join("");
      ...
      m.querySelectorAll("[data-save]").forEach(b=>b.onclick=async()=>{
        const id=b.dataset.save, before=(data||[]).find(x=>x.id===id), name=(m.querySelector(`[data-name="${id}"]`)?.value||"").trim();
        if(name.length<2){alert("Nama Karyawan minimal 2 karakter.");return}
        b.disabled=true;b.textContent="Menyimpan...";
        try{
          if(name!==(before?.display_name||"")){
            const {error:nameError}=await sb.rpc("bf_owner_update_display_name",{p_user_id:id,p_display_name:name});
            if(nameError)throw nameError;
          }
          if(before?.role!=="owner"){
            const role=m.querySelector(`[data-role="${id}"]`).value, active=m.querySelector(`[data-active="${id}"]`).checked;
            if(role!==before.role||active!==before.active){
          ...
          openUsers();
        }catch(error){alert(error?.message||error);b.disabled=false;b.textContent="Simpan"}
      });
    }
    ...
    async function openPermissions(){
      ...
      for(const role of roles){
        ...
        for(const [key,cfg] of activeEntries){
          if(cfg.ownerOnly || cfg.driverOnly) continue;
      ...
      const m=modal("Pengaturan Hak Akses",html+`<div class="bf-perm-note"><b>Catatan:</b> Hak akses khusus Owner tidak ditawarkan kepada Admin/Operator. Hak Driver/Supir bersifat tetap dan sangat terbatas, sehingga tidak diedit di panel ini. Nama yang tampil di sini adalah bahasa manusia; sistem tetap menyimpan permission key internal yang sama.</div><button id="bf-save-perm" class="bf-auth-btn" style="background:#0d1b3e;color:#fff">Simpan Hak Akses</button>`,920);
