/*
  ...
  (async function(){
    ...
    const DEFAULTS = {
      owner: window.BFPermissionRegistry.defaults("owner"),
      admin: window.BFPermissionRegistry.defaults("admin"),
      operator: window.BFPermissionRegistry.defaults("operator")
    };
    ...
    function bridgeLegacySession(){
      ...
      const permissionsLegacy=window.BFPermissionRegistry.legacyPermissions(profile.role,has);
      const legacy={
    ...
    async function openPermissions(){
      ...
      for(const role of roles){
        ...
        html+=`<div style="border:1px solid #e5e7eb;border-radius:12px;padding:12px;margin-bottom:12px">
          ...
          ${window.BFPermissionRegistry.keys().map(k=>`<label style="font-size:11px"><input type="checkbox" data-p="${role}:${k}" ${p[k]?"checked":""}> ${esc(window.BFPermissionRegistry.label(k))}<small style="display:block;color:#94a3b8">${esc(k)}</small></label>`).join("")}
          </div></div>`;
      }
