...
$("bfTallyCombinedPdf").onclick=()=>{
  ...
  const masuk=window.BFPeriodFilter?BFPeriodFilter(arr(read("bf_masuk_v26",[]))):arr(read("bf_masuk_v26",[]));
  const keluar=window.BFPeriodFilter?BFPeriodFilter(arr(read("bf_keluar_v26",[]))):arr(read("bf_keluar_v26",[]));
  ...
  const setoran=window.BFPeriodFilter?BFPeriodFilter(arr(read("bf_note_setoran_v26",[]))):arr(read("bf_note_setoran_v26",[]));
  const pengeluaran=window.BFPeriodFilter?BFPeriodFilter(arr(read("bf_note_pengeluaran_v26",[]))):arr(read("bf_note_pengeluaran_v26",[]));
  const sembako=window.BFPeriodFilter?BFPeriodFilter(arr(read("bf_note_sembako_v26",[]))):arr(read("bf_note_sembako_v26",[]));
  ...
  const newPage=(titleText="")=>{
    ...
    text(M,H-25,"BINTANG FROZEN",13,white,"/F2");
  ...
  text(M,H-55,"BINTANG FROZEN",28,white,"/F2"); text(M,H-82,"LAPORAN GABUNGAN OPERASIONAL",18,[218,226,240],"/F2");
  ...
  text(M,55,"Dicetak otomatis dari Bintang Frozen",8,muted,"/F1"); text(W-M-150,55,new Date().toLocaleString("id-ID"),8,muted,"/F1");
  pages.push(ops);ops=[];
