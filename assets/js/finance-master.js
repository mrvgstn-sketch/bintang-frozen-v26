...
function financeDateLabel(value){
  const m=String(value||"").match(/^(\d{4})-(\d{2})-(\d{2})$/);if(!m)return String(value||"-");
  const d=new Date(Number(m[1]),Number(m[2])-1,Number(m[3]));
  return new Intl.DateTimeFormat("id-ID",{day:"numeric",month:"long",year:"numeric"}).format(d);
}
function depositMethodInfo(row){
  const raw=String(row?.metode??row?.via_bank??"").trim();
  if(!raw)return {key:"__missing__",label:"Metode Tidak Tercatat"};
  return {key:raw.toLocaleLowerCase("id-ID"),label:raw};
}
function groupDepositRows(rows){
  const byDate=new Map();
  sortFinanceRows(rows).forEach(row=>{
    const date=String(row.tanggal||"").slice(0,10)||"__missing_date__";
    if(!byDate.has(date))byDate.set(date,{date,total:0,count:0,methodMap:new Map()});
    const dateGroup=byDate.get(date),method=depositMethodInfo(row);
    dateGroup.total+=n(row.nominal);dateGroup.count++;
    if(!dateGroup.methodMap.has(method.key))dateGroup.methodMap.set(method.key,{key:method.key,label:method.label,total:0,count:0,rows:[]});
    const methodGroup=dateGroup.methodMap.get(method.key);
    if(method.key!=="__missing__"){const raw=String(row?.metode??row?.via_bank??"").trim();if(raw&&raw===raw.toLocaleUpperCase("id-ID")&&methodGroup.label!==methodGroup.label.toLocaleUpperCase("id-ID"))methodGroup.label=raw}
    methodGroup.total+=n(row.nominal);methodGroup.count++;methodGroup.rows.push(row);
  });
  return [...byDate.values()].sort((a,b)=>b.date.localeCompare(a.date)).map(group=>({
    date:group.date,total:group.total,count:group.count,
    methods:[...group.methodMap.values()].sort((a,b)=>{
      if(a.key==="__missing__")return 1;if(b.key==="__missing__")return -1;
      return a.label.localeCompare(b.label,"id",{sensitivity:"base"});
    })
  }));
}
function depositHistoryMarkup(filtered){
  const groups=groupDepositRows(filtered);
  if(!groups.length)return `<div class="bf-deposit-empty">Belum ada setoran pada periode ini.</div>`;
  return `<div class="bf-deposit-history">${groups.map(dateGroup=>`<section class="bf-deposit-date-group">
    <header class="bf-deposit-date-head"><div><small>TANGGAL</small><h3>${esc(dateGroup.date==="__missing_date__"?"Tanggal Tidak Tercatat":financeDateLabel(dateGroup.date))}</h3></div><div class="bf-deposit-date-total"><strong>${rupiah(dateGroup.total)}</strong><span>${dateGroup.count.toLocaleString("id-ID")} transaksi</span></div></header>
    <div class="bf-deposit-method-list">${dateGroup.methods.map(method=>`<section class="bf-deposit-method-group">
      <header class="bf-deposit-method-head"><div><b>${esc(method.label)}</b><span>${method.count.toLocaleString("id-ID")} setoran</span></div><strong>${rupiah(method.total)}</strong></header>
      <div class="bf-deposit-transactions">${method.rows.map(x=>`<article class="bf-deposit-row" data-record-id="${esc(x.id)}">
        <div class="bf-deposit-main"><b>${esc(x.customer||"-")}</b><strong>${rupiah(x.nominal)}</strong></div>
        <div class="bf-deposit-meta"><span><i>Metode</i>${esc(x.metode||x.via_bank||"Metode Tidak Tercatat")}</span><span><i>Keterangan</i>${esc(x.keterangan||"-")}</span></div>
        <div class="bf-deposit-actions">${photoArray(x.bukti_fotos||x.bukti_foto).length?`<button class="bfcrud-btn view-photo" data-id="${esc(x.id)}">📷 Lihat Bukti</button>`:""}<button class="bfcrud-btn edit" data-id="${esc(x.id)}">Edit</button><button class="bfcrud-btn danger del" data-id="${esc(x.id)}">Hapus</button></div>
      </article>`).join("")}</div>
    </section>`).join("")}</div>
  </section>`).join("")}</div>`;
}
...
async function saveFinanceOwnerReview(key,id,ownerReview){
  if(user().role!=="owner")return false;
  if(!["bf_note_setoran_v26","bf_expenses","bf_note_pengeluaran_v26"].includes(String(key||"")))return false;
  const all=list(key),i=all.findIndex(x=>String(x.id||x._bf_uid||"")===String(id||""));if(i<0)return false;
  const old=all[i],next={...old,owner_review:ownerReview??null};all[i]=next;write(key,all);audit(ownerReview?"report_review_completed":"report_review_reopened",key==="bf_note_setoran_v26"?"setoran":"pengeluaran",id,old,next,{review_scope:"finance_record"});
  if(window.BFCloud?.push){const ok=await window.BFCloud.push();if(ok===false)console.warn("[Bintang Frozen] Owner review tersimpan lokal tetapi sinkronisasi cloud belum terkonfirmasi",key,id)}
  return true;
}
window.BFFinanceMaster={...(window.BFFinanceMaster||{}),saveOwnerReview:saveFinanceOwnerReview};
