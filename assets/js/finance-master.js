...
function deposit(){
  if(!can("view_finance")&&!can("view_note"))return deny("view_finance");
  const KEY="bf_note_setoran_v23c";let rows=list(KEY).filter(x=>!x.deleted_at),editing=null;const p=page("Keuangan — Setoran","Catatan setoran dengan bukti transfer / slip jika ada");const c=p.querySelector(".bfcrud-content");
  function render(r={}){
...
function grocery(){
  if(!can("view_finance")&&!can("view_note"))return deny("view_finance");
  const KEY="bf_note_sembako_v23c";let rows=list(KEY).filter(x=>!x.deleted_at),editing=null;const p=page("Keuangan — Catatan Pembelian Sembako","Satu supplier / tempat beli dapat mempunyai banyak barang. Tidak masuk Data Master Produk.");const c=p.querySelector(".bfcrud-content");
...
function commission(){
  ...
  const tx=list("bf_keluar_v23_manual").filter(x=>!x.deleted_at);
