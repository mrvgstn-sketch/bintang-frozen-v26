...
function commission(){
  ...
  const rate=name=>n(employees.find(x=>x.active!==false&&String(x.name||x.email||"").toLowerCase()===String(name||"").toLowerCase())?.commission||employees.find(x=>x.active!==false&&String(x.name||x.email||"").toLowerCase()===String(name||"").toLowerCase())?.commission_per_kg||0);
  const tx=list("bf_keluar_v26").filter(x=>!x.deleted_at);
  const rows=tx.flatMap(r=>DM.customerGroups(r).map(g=>{const per=g.marketing?(n(g.commission_per_kg)||rate(g.marketing)):0,kg=DM.groupTotal(g);return {...r,_customer:g.customer||"-",_marketing:g.marketing,_kg:kg,_rate:per,_commission:kg*per}})).filter(r=>r._marketing&&r._rate>0);
  const p=page("Keuangan — Komisi","Dihitung per Customer: total kg × Komisi/Kg Marketing");const c=p.querySelector(".bfcrud-content");
  c.innerHTML=`<div class="bfcrud-card"><small style="font-weight:800;color:#64748b">TOTAL KOMISI</small><div class="bfcrud-money">${rupiah(rows.reduce((s,x)=>s+x._commission,0))}</div><div style="font-size:11px;color:#64748b">Customer yang dipilih Tanpa Marketing tidak mendapat komisi.</div></div><div class="bfcrud-card"><div class="bfcrud-table-wrap"><table class="bfcrud-table"><thead><tr><th>Barang Keluar</th><th>Tanggal</th><th>Customer</th><th>Marketing</th><th>Total Kg</th><th>Komisi/Kg</th><th>Total</th></tr></thead><tbody>${rows.slice().reverse().map(x=>`<tr><td><b>${esc(x.transaction_no||"-")}</b></td><td>${esc(x.tanggal||"-")}</td><td>${esc(x._customer)}</td><td>${esc(x._marketing)}</td><td>${x._kg.toLocaleString("id-ID")} kg</td><td>${rupiah(x._rate)}/kg</td><td><b>${rupiah(x._commission)}</b></td></tr>`).join("")||'<tr><td colspan="7">Belum ada transaksi dengan Marketing.</td></tr>'}</tbody></table></div></div>`;
}
