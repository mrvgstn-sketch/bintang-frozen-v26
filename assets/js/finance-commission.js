...
function finance(kind){
  ...
  const cfg={
    expense:{title:"Pengeluaran",keys:["bf_note_pengeluaran_v23b","bf_note_pengeluaran_v23c","bf_expenses"]},
    deposit:{title:"Setoran",keys:["bf_note_setoran_v23b","bf_note_setoran_v23c"]},
    grocery:{title:"Sembako",keys:["bf_note_sembako_v23c"]}
  }[kind];
...
function commission(){
  ...
  const keluar=arr("bf_keluar_v23_manual").filter(r=>!r.deleted_at);
...
function exports(){
  ...
  p.querySelector("#epdf").onclick=()=>document.getElementById("bf25CombinedPdf")?.click();
  p.querySelector("#ecsv").onclick=()=>document.getElementById("bf25Combined")?.click();
  p.querySelector("#eprint").onclick=()=>document.getElementById("bf25Print")?.click();
}
function backup(){
  if(!can("backup") && (window.BFCurrentUser?.()?.profile?.role||"")!=="owner")return deny("backup");
  document.getElementById("bf25Json")?.click();
