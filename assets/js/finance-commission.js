...
function commission(){
  ...
  const rows=keluar.flatMap(r=>{
    const legacy=r.marketing||r.staff||r.sales||r.employee||"";
    const groups=Array.isArray(r.customers)&&r.customers.length?r.customers:[{customer:r.customer||"",marketing:legacy,items:[{timbangan:Array.isArray(r.timbangan)?r.timbangan:[]}]}];
    return groups.map(g=>{
      const marketing=g.marketing||g.marketingNama||legacy||"";
      const rate=num(g.commission_per_kg||employeeRate(marketing));
      const totalKg=(g.items||[]).flatMap(i=>i.timbangan||i.weights||[]).reduce((a,b)=>a+num(b),0);
      return {...r,_customer:g.customer||g.name||r.customer||"",_marketing:marketing,_rate:rate,_groupKg:totalKg,_commission:totalKg*rate};
    });
  }).filter(r=>r._marketing || r._rate>0);
  const total=rows.reduce((s,r)=>s+r._commission,0);
  const p=page("Komisi","Dihitung per Customer dari Barang Keluar × Komisi/Kg Marketing");
  p.querySelector("#bf-fin-content").innerHTML=`<div class="bf-fin-card"><span style="font-size:10px;color:#64748b;font-weight:800;text-transform:uppercase">Total Komisi</span><div class="bf-commission-total">${money(total)}</div><small style="color:#64748b">Tidak perlu input komisi dua kali.</small></div><div class="bf-fin-card"><div style="overflow:auto"><table class="bf-fin-table"><thead><tr><th>No Transaksi</th><th>Tanggal</th><th>Marketing</th><th>Total Kg</th><th>Komisi/Kg</th><th>Komisi</th></tr></thead><tbody>${rows.slice().reverse().map(r=>`<tr><td><b>${esc(r.transaction_no||"-")}</b></td><td>${esc(r.tanggal||r.date||"-")}</td><td>${esc(r._marketing||"-")}</td><td>${r._groupKg.toLocaleString("id-ID")} kg</td><td>${money(r._rate)}</td><td><b>${money(r._commission)}</b></td></tr>`).join("")||'<tr><td colspan="6">Belum ada transaksi dengan Marketing.</td></tr>'}</tbody></table></div></div>`;
}
