(function(){
  ...
  function showFatal(message){
    try{
      ...
      box.innerHTML='<div style="width:min(460px,92vw);background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.15);border-radius:18px;padding:20px;text-align:center"><b>BINTANG FROZEN</b><div style="margin-top:10px;font-size:12px;color:#fecaca;word-break:break-word">'+String(message||"Aplikasi gagal dimuat")+'</div><button onclick="location.reload()" style="margin-top:15px;border:0;border-radius:10px;padding:10px 14px;background:#ffcc00;color:#0d1b3e;font-weight:900">Muat Ulang</button></div>';
    }catch(_){}
  }

  // Tangkap error startup sejak detik pertama.
  ...
  window.addEventListener("unhandledrejection",function(ev){
    ...
    if(!document.getElementById("root")?.children?.length){
      showFatal(ev?.reason?.message||ev?.reason||"Promise startup gagal");
    }
  });

  window.addEventListener("bf:startup-error",function(ev){
    if(!document.getElementById("root")?.children?.length){
      showFatal(ev?.detail?.message||"Aplikasi gagal dimuat");
    }
  });
