...
function outgoingMaterial(g={}){return {group_id:String(g.group_id||""),customer:String(g.customer||""),marketing:String(g.marketing||""),fulfillment_method:String(g.fulfillment_method||"delivery"),driver_id:String(g.driver_id||""),driver_name_snapshot:String(g.driver_name_snapshot||""),payment_method:String(g.payment_method||""),delivery_status:String(g.delivery_status||""),cash_status:String(g.cash_status||""),pos_status:String(g.pos_status||""),pos_snapshot:g.pos_snapshot??null,items:(g.items||[]).map(itemMaterial),delivery_proofs:(Array.isArray(g.delivery_proofs)?g.delivery_proofs:[]).map(proofMaterial)}}
...
function depositMaterial(r={}){return {tanggal:String(r.tanggal||r.date||""),customer:String(r.customer||r.namaCustomer||""),nominal:money(r.nominal??r.total??r.jumlah??0),metode:String(r.metode||r.via_bank||r.via||""),keterangan:String(r.keterangan||""),bukti_fotos:photos(r.bukti_fotos||r.bukti_foto)}}
function expenseMaterial(r={}){return {tanggal:String(r.tanggal||r.date||""),kategori:String(r.kategori||r.jenis||""),nominal:money(r.nominal??r.total??r.jumlah??0),metode:String(r.metode||r.via||""),keterangan:String(r.keterangan||""),bukti_fotos:photos(r.bukti_fotos||r.bukti_foto)}}
const outgoingFingerprint=g=>hashMaterial(outgoingMaterial(g));
const incomingFingerprint=tx=>hashMaterial(incomingMaterial(tx));
const depositFingerprint=r=>hashMaterial(depositMaterial(r));
const expenseFingerprint=r=>hashMaterial(expenseMaterial(r));
const financeIdentity=r=>String(r?.id||r?._bf_uid||"");
...
function build({from="",to=""}={}){
  ...
  const expensePrimary=list("bf_expenses").map(r=>({...r,__bf_source_key:"bf_expenses"})),expenseLegacy=list("bf_note_pengeluaran_v26").map(r=>({...r,__bf_source_key:"bf_note_pengeluaran_v26"}));
  const pengeluaran=dedupe([...expensePrimary,...expenseLegacy],"exp").filter(r=>inRange(r,from,to));
  ...
  const setoranDetail=setoran.map(row=>{const fp=depositFingerprint(row);return {row,identity:financeIdentity(row),sourceKey:"bf_note_setoran_v26",review:reviewState(row.owner_review,fp),reviewFingerprint:fp}});
  const pengeluaranDetail=pengeluaran.map(row=>{const fp=expenseFingerprint(row);return {row,identity:financeIdentity(row),sourceKey:row.__bf_source_key||"bf_expenses",review:reviewState(row.owner_review,fp),reviewFingerprint:fp}});
  return {from,to,masuk:incoming,keluar:outgoing,setoran,pengeluaran,sembako,komisi:commissions,setoranDetail,pengeluaranDetail,helpers:{dateOf,actor,timeOf,money,photos,txIdentity,outgoingFingerprint,incomingFingerprint,depositFingerprint,expenseFingerprint,reviewState,variance}};
}
window.BFReportingEngine={build,txIdentity,outgoingFingerprint,incomingFingerprint,depositFingerprint,expenseFingerprint,reviewState,_outgoingMaterial:outgoingMaterial,_incomingMaterial:incomingMaterial,_depositMaterial:depositMaterial,_expenseMaterial:expenseMaterial};
})();
