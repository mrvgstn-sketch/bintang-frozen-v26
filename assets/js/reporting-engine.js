...
function build({from="",to=""}={}){
  ...
  const outgoing=keluar.map(tx=>({tx,groups:DM.customerGroups(tx).map(g=>{const rateKnown=DM.own(g,"commission_per_kg")&&DM.num(g.commission_per_kg)>0,r=rateKnown?DM.num(g.commission_per_kg):0;return {...g,items:(g.items||[]).map(i=>({...i,tally:DM.itemWeights(i),totalKg:DM.itemTotal(i)})),totalKg:DM.groupTotal(g),commissionRateKnown:rateKnown,commissionPerKg:g.marketing&&rateKnown?r:0,totalKomisi:g.marketing&&rateKnown?DM.groupTotal(g)*r:0}})}));
