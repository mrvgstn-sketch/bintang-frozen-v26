...
function quickAction(show,label,sub,action){
  return show?`<button type="button" class="bf-dashboard-shortcut" data-bf-dashboard-action="${esc(action)}"><b>${esc(label)}</b><small>${esc(sub)}</small></button>`:"";
}
function renderDashboardEnhancements(){
  const existing=document.getElementById("bf-dashboard-enhancements");
  if(window.BFNativeRoute!=="dashboard"){existing?.remove();return}
  const host=dashboardHost();if(!host)return;
  const role=String(window.BFCurrentUser?.()?.profile?.role||"").toLowerCase();
  const d=dataset(window.BFCore.today(),window.BFCore.today()), alerts=issues(d);
  let panel=existing;
  if(!panel){panel=document.createElement("section");panel.id="bf-dashboard-enhancements";panel.className="bf-dashboard-enhancements";host.prepend(panel)}
  panel.innerHTML=`<div class="bf-dashboard-shortcuts">
    ${quickAction(can("view_in"),"📥 Barang Masuk","Catat penerimaan & Tally","transactions-in")}
    ${quickAction(can("view_out"),"📤 Barang Keluar","Customer, Marketing & Tally","transactions-out")}
    ${quickAction(can("view_finance")||can("view_note"),"💰 Keuangan","Pengeluaran, Setoran, Sembako","finance")}
    ${quickAction(can("view_reports")||can("view_history"),"📊 Laporan","Ringkasan & pemeriksaan","reports")}
  </div>
  ${role==="owner"?`<div class="bf-owner-dashboard-panel"><div class="bf-owner-dashboard-title"><b>Kontrol Owner — Hari Ini</b><button type="button" data-bf-dashboard-action="reports">Buka Laporan Rinci</button></div><div class="bf-report-metrics">${metric("Masuk",d.kgIn.toLocaleString("id-ID")+" kg",d.masuk.length+" transaksi","good")}${metric("Keluar",d.kgOut.toLocaleString("id-ID")+" kg",d.keluar.length+" transaksi","info")}${metric("Setoran",money(d.depositTotal),d.deposits.length+" catatan","good")}${metric("Perlu Dicek",String(alerts.length),"indikator berbasis data",alerts.length?"warn":"good")}</div></div>`:""}`;
  panel.querySelectorAll("[data-bf-dashboard-action]").forEach(b=>b.onclick=()=>{
    const a=b.dataset.bfDashboardAction, map={
      "transactions-in":"BFOpenTransactionsIn","transactions-out":"BFOpenTransactionsOut",
      "finance":"BFOpenFinanceExpense","reports":"BFOpenReports"
    };
    const fn=map[a];if(fn&&typeof window[fn]==="function")window[fn]();
  });
}
const previousRouteHook=window.BFOnNativeRoute;
window.BFOnNativeRoute=function(route){
  previousRouteHook?.(route);
  if(route==="dashboard")requestAnimationFrame(renderDashboardEnhancements);
  else document.getElementById("bf-dashboard-enhancements")?.remove();
};

window.BFRenderDashboardEnhancements=renderDashboardEnhancements;
