...
function menuHTML(){
  ...
  return '<div class="bf-app-menu-head">Menu Bintang Frozen</div>'+ section('🏠 DASHBOARD',item(true,button('🏠 Dashboard','Pusat kontrol aplikasi','route','dashboard')))+
    ...
    section('💰 KEUANGAN',item(can('view_finance')||can('view_note'),button('💰 Pengeluaran','Catatan pengeluaran','action','finance-expense'))+item(can('view_finance')||can('view_note'),button('🏦 Setoran','Catatan setoran','action','finance-deposit'))+item(owner||admin,button('🧾 Nota & Dana Titipan','Titipan Customer • Cicilan • Rekonsiliasi Kas','action','nota-dana-titipan'))+item(owner&&can('view_commission'),button('💸 Komisi','Otomatis per Customer/Marketing','action','commission'))+item(can('view_finance')||can('view_note'),button('🛒 Sembako','Catatan sembako','action','finance-grocery')))+
...
function loadNotaDanaTitipan(){
  if(typeof window.BFOpenNotaDanaTitipan==='function'){window.BFOpenNotaDanaTitipan();return Promise.resolve(true)}
  if(window.__bfNotaDanaTitipanLoading)return window.__bfNotaDanaTitipanLoading;
  window.__bfNotaDanaTitipanLoading=new Promise((resolve,reject)=>{
    const s=document.createElement('script');s.src='assets/js/nota-dana-titipan.js?v=20260822-uat1';s.async=true;
    s.onload=()=>{if(typeof window.BFOpenNotaDanaTitipan==='function'){window.BFOpenNotaDanaTitipan();resolve(true)}else reject(new Error('Modul Nota & Dana Titipan gagal diinisialisasi.'))};
    s.onerror=()=>reject(new Error('Modul Nota & Dana Titipan gagal dimuat.'));
    document.head.appendChild(s);
  }).finally(()=>{window.__bfNotaDanaTitipanLoading=null});
  return window.__bfNotaDanaTitipanLoading;
}
function bind(wrap){
