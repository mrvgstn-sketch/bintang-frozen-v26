...
function close(wrap=activeWrap){
  if(!wrap)return;
  const btn=wrap.querySelector('.bf-app-menu-btn'),pop=wrap.querySelector('.bf-app-menu-pop');
  pop?.classList.remove('bf-show');btn?.classList.remove('bf-open');btn?.setAttribute('aria-expanded','false');
}
function position(btn,pop){
  const r=btn.getBoundingClientRect(),vw=document.documentElement.clientWidth||innerWidth,margin=12,width=Math.min(760,Math.max(280,vw-margin*2));
  let left=Math.min(Math.max(margin,r.right-width),vw-margin-width);if(vw<768)left=margin;
  pop.style.width=(vw<768?vw-margin*2:width)+'px';pop.style.left=Math.max(margin,left)+'px';
  pop.style.top=Math.min(r.bottom+8,(window.visualViewport?.height||window.innerHeight)-80)+'px';
}
function bind(wrap){
  if(!wrap || wrap.id!=="bf-app-menu-anchor")return false;
  activeWrap=wrap;
  if(wrap.dataset.bfMenuBound==="1")return true;
  const btn=wrap.querySelector('.bf-app-menu-btn'),pop=wrap.querySelector('.bf-app-menu-pop');
  if(!btn||!pop)return false;
  wrap.dataset.bfMenuBound="1";
  btn.addEventListener('click',e=>{
    e.stopPropagation();
    if(pop.classList.contains('bf-show'))return close(wrap);
    pop.innerHTML=menuHTML();position(btn,pop);pop.classList.add('bf-show');btn.classList.add('bf-open');btn.setAttribute('aria-expanded','true');
  });
...
if(!window.__bfMenuGlobalListeners){
  window.__bfMenuGlobalListeners=true;
  document.addEventListener('click',e=>{const wrap=activeWrap;if(wrap&&!wrap.contains(e.target))close(wrap)});
