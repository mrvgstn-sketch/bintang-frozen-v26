(function(){
  "use strict";
  function menuHTML(){
    var can=function(p){try{return window.BFCan?window.BFCan(p):true}catch(_){return true}};
    ...
    return '<div class="bf-final-menu-head">Menu Bintang Frozen</div>'+
      ...
      item(can("view_history")||can("request_history"),'<button class="bf-final-menu-item" data-route="histori">📚 Histori<small>Read-only, edit dengan izin</small></button>')+
      item(can("view_reports")||can("export_pdf")||can("export_csv"),'<button class="bf-final-menu-item" data-custom-action="reports">📊 Laporan<small>Filter & ringkasan</small></button>')+
      item(can("export_pdf")||can("export_csv"),'<button class="bf-final-menu-item" data-custom-action="exports">📄 Export PDF / CSV<small>Laporan & data</small></button>')+
      '</div></div>'+
      ...
      item(can("backup")||owner,'<button class="bf-final-menu-item" data-custom-action="backup">💾 Backup<small>Backup darurat Owner</small></button>')+
  ...
  function wire(pop,btn,wrap){
    ...
    pop.addEventListener('click',function(e){
      ...
      if(item.dataset.action){
        if(item.dataset.action==='backup'&&!(window.BFCan&&BFCan('backup'))){alert('Backup Darurat JSON hanya untuk Owner.');return;}
        var ids={pdf:'bf25CombinedPdf',csv:'bf25Combined',print:'bf25Print',backup:'bf25Json'},target=document.getElementById(ids[item.dataset.action]);
