...
function menuHTML(){
  ...
  return '<div class="bf-app-menu-head">Menu Bintang Frozen</div>'+ section('🏠 RINGKASAN',item(true,button('🏠 Ringkasan','Pusat kendali aplikasi','route','dashboard')))+
    section('📦 TRANSAKSI',item(can('view_in'),button('📥 Barang Masuk','Transaksi & lembar timbang','action','transactions-in'))+item(can('view_out'),button('📤 Barang Keluar','Transaksi & lembar timbang','action','transactions-out'))+item(can('view_tally'),button('⚖️ Lembar Timbang','Barang Masuk / Barang Keluar','action','tally')))+
    section('💰 KEUANGAN',item(can('view_finance')||can('view_note'),button('💰 Pengeluaran','Catatan pengeluaran','action','finance-expense'))+item(can('view_finance')||can('view_note'),button('🏦 Setoran','Catatan setoran Customer • mendukung pembayaran kombinasi','action','finance-deposit'))+item(owner||admin,button('🧮 Rekonsiliasi Kas Harian','Pengeluaran otomatis • Admin ajukan • Owner periksa','action','cash-reconciliation'))+item(owner||admin,button('💳 Dana Customer','Komisi Customer • Dana Titipan • Pengembalian Dana','action','customer-funds'))+item(owner||admin,button('🧾 Nota & Dana Titipan','Titipan Customer • Cicilan • Pembayaran Titipan','action','nota-dana-titipan'))+item(owner||admin,button('🛡 Kontrol Dana Titipan','Koreksi • Pembatalan • Riwayat Customer','action','nota-dana-controls'))+item(owner&&can('view_commission'),button('💸 Komisi Marketing','Komisi Marketing per kg dari Barang Keluar','action','commission'))+item(can('view_finance')||can('view_note'),button('🛒 Sembako','Catatan sembako','action','finance-grocery')))+
    ...
    section('📊 LAPORAN',item(can('view_history')||can('request_history'),button('📚 Histori','Transaksi dan perubahan terakhir','action','history-detail'))+item(can('view_reports'),button('📊 Laporan Detail','Semua detail transaksi dan lembar timbang','action','reports'))+item(can('export_pdf')||can('export_csv'),button('📄 Ekspor PDF / CSV','Pilih periode dan kategori','action','exports')))+
    section('🚚 PENGANTARAN',item(owner,button('📍 Lokasi Driver','Posisi terakhir Driver • khusus Owner','action','driver-location')))+
    section('👥 MANAJEMEN',item(can('view_employees'),button('👥 Karyawan','Manajemen karyawan','action','employees'))+item(can('manage_users'),button('👤 Pengguna','Akun Google & peran','owner','users'))+item(can('manage_permissions'),button('🔐 Hak Akses','Hak akses per fitur','owner','permissions'))+item(can('approve_history'),button('🔔 Izin Histori','Setujui / tolak permintaan','owner','requests'))+item(owner&&can('view_audit'),button('📋 Riwayat Aktivitas','Aktivitas pengguna','owner','audit')))+
    section('⚙️ SISTEM',item(can('view_settings')||owner,button('⚙️ Pengaturan','Konfigurasi aplikasi','route','pengaturan'))+item(can('backup')||owner,button('☁️ Cadangan Google Drive','Cadangan lengkap & pemulihan','action','backup'))+item(owner,button('🗑 Data Terhapus','Pulihkan / hapus permanen','owner','deleted')));
}
...
function loadScriptOnce(src,key,readyFn,open=true){
  if(typeof window[readyFn]==='function'){if(open)window[readyFn]();return Promise.resolve(true)}
  if(window[key])return window[key];
  window[key]=new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=src;s.async=true;s.onload=()=>{if(typeof window[readyFn]==='function'){if(open)window[readyFn]();resolve(true)}else reject(new Error('Modul gagal diinisialisasi.'))};s.onerror=()=>reject(new Error('Modul gagal dimuat.'));document.head.appendChild(s)}).finally(()=>{window[key]=null});
  return window[key];
}
function ensureCashReconciliation(){return loadScriptOnce('assets/js/cash-reconciliation.js?v=20260823-r13-cash1','__bfCashReconciliationLoading','BFOpenCashReconciliation',false)}
function loadCashReconciliation(){return loadScriptOnce('assets/js/cash-reconciliation.js?v=20260823-r13-cash1','__bfCashReconciliationLoading','BFOpenCashReconciliation',true)}
function loadCanonicalSetoran(){return loadScriptOnce('assets/js/setoran-canonical-split.js?v=20260823-r13-split1','__bfCanonicalSetoranLoading','BFOpenCanonicalSetoran',true)}
async function loadCustomerFunds(){await ensureCashReconciliation();return loadScriptOnce('assets/js/customer-funds.js?v=20260822-uat1','__bfCustomerFundsLoading','BFOpenCustomerFunds',true)}
async function loadNotaDanaTitipan(){
  await ensureCashReconciliation();
  await loadScriptOnce('assets/js/nota-dana-titipan-signature-hardening.js?v=20260822-uat2','__bfNotaSignatureLoading','BFNotaSignatureStrokeCount',false);
  return loadScriptOnce('assets/js/nota-dana-titipan.js?v=20260822-uat2','__bfNotaDanaTitipanLoading','BFOpenNotaDanaTitipan',true);
}
function loadNotaDanaControls(){return loadScriptOnce('assets/js/nota-dana-titipan-controls.js?v=20260822-uat2','__bfNotaDanaControlsLoading','BFOpenNotaDanaTitipanControls',true)}
function bind(wrap){
  ...
  pop.addEventListener('click',e=>{
    ...
    if(it.dataset.action==='cash-reconciliation'){close(wrap);loadCashReconciliation().catch(err=>{console.error(err);alert(err.message||String(err))});return}
    if(it.dataset.action==='customer-funds'){close(wrap);loadCustomerFunds().catch(err=>{console.error(err);alert(err.message||String(err))});return}
