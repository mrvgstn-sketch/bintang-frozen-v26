...
function install(){
  const host=document.getElementById('bf-shell-menu');if(!host||document.querySelector('.bf-app-menu-wrap'))return;
  const wrap=document.createElement('span');wrap.className='bf-app-menu-wrap';wrap.innerHTML='<button type="button" class="bf-app-menu-btn">☰ <span>MENU</span></button><div class="bf-app-menu-pop"></div>';host.appendChild(wrap);
  ...
  const position=()=>{const r=btn.getBoundingClientRect(),vw=document.documentElement.clientWidth||innerWidth,margin=12,width=Math.min(760,Math.max(280,vw-margin*2));let left=Math.min(Math.max(margin,r.right-width),vw-margin-width);if(vw<768)left=margin;pop.style.width=(vw<768?vw-margin*2:width)+'px';pop.style.left=Math.max(margin,left)+'px';pop.style.top=Math.min(r.bottom+8,(window.visualViewport?.height||window.innerHeight)-80)+'px'};
