...
const js=read("assets/js/customer-funds-control.js"),core=read("assets/js/core.js"),menu=read("assets/js/menu.js"),sql=read("supabase/migrations/20260822_r13_customer_funds_control.sql");
...
assert(sql.includes("for update"),"concurrency row locking missing");
...
assert(sql.includes("security definer"),"guarded RPC boundary missing");
assert(!/insert\s+into\s+public\.(?:bf_keluar|bf_masuk|bf_state_items)/i.test(sql),"migration must not become sales/inventory writer");
